...
export default function SettingsEmployeePage() {
    ...
    useEffect(() => {
        fetch("/api/employeer/settingsemployee", { cache: "no-store" })
            .then(res => res.json())
            .then(data => {
                if (data.success && data.data) {
                    ...
                    setAge(emp.user?.age !== null && emp.user?.age !== undefined ? String(emp.user.age) : "");
                    setGender(emp.user?.gender || "");
                    setName(`${emp.firstName || ""} ${emp.lastName || ""}`.trim());
                    setAvatar(emp.user?.avatar || null);
                }
            })
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/employeer/settingsemployee", {
                method: "PATCH",
    ...
    return (
        <div className="p-6 md:p-12 lg:p-16 space-y-10 relative">
            ...
            <div className="flex items-center gap-6 p-6 bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-white/5 shadow-sm max-w-sm">
                <div className="relative shrink-0">
                    <div className="w-20 h-20 rounded-2xl bg-[#bca086] flex items-center justify-center text-white text-2xl font-black shadow-lg overflow-hidden">
                        {avatar
                            ...
                            : <span>{firstName?.charAt(0)?.toUpperCase() || "E"}</span>
                        }
                    </div>
                ...
                <div>
                    <p className="font-black text-slate-800 dark:text-white text-sm">{name || "Employé"}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{position || "Employé"}</p>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-sm p-8 md:p-10 space-y-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-medium mt-0.5">Prénom, nom, téléphone, âge et genre</p>
                        </div>
                    </div>
                    ...
                    {loadingProfile ? (
                        ...
                        <form onSubmit={handleSaveProfile} className="space-y-5">
                            ...
                            <Field label="Poste" value={position} onChange={setPosition} placeholder="Ex : Photographe" />
                            <Field label="Numéro de téléphone" value={phone} onChange={setPhone} type="tel" placeholder="+33 6 00 00 00 00" />
                            <Field label="Âge" value={age} onChange={setAge} type="number" placeholder="Ex : 28" />
