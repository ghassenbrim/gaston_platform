...
const dashboardItems = [
    {
        title: "Mes photos",
        description: "Gérez et visualisez vos photographies.",
        icon: (
            <svg
    ...
    {
        title: "Rendez-Vous",
        description: "Consultez vos réservations à venir.",
        icon: (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
    ...
    {
        title: "Contrat",
        description: "Accédez à vos documents et contrats.",
        icon: (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
    ...
    {
        title: "Messagerie",
        description: "Discutez avec GASTON.",
        icon: (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
            ...
            >
                <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M7.5 8.25h9m-9 3h9m-9 3h3m-6.75 3h1.5l2.25 2.25 2.25-2.25h7.5a2.25 2.25 0 0 0 2.25-2.25V4.5a2.25 2.25 0 0 0-2.25-2.25h-15a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 4.5 20.25Z"
                />
            </svg>
        ),
...
export default function UserLayout({
    ...
    // Translate dashboard items based on language
    const translatedDashboardItems = useMemo(() => {
        const translations = [
            { titleKey: "nav_photos", descKey: "nav_photos_desc" },
            { titleKey: "nav_favorites", descKey: "nav_favorites_desc" },
            { titleKey: "nav_appointments", descKey: "nav_appointments_desc" },
            { titleKey: "nav_contract", descKey: "nav_contract_desc" },
            { titleKey: "nav_messaging", descKey: "nav_messaging_desc" },
            undefined, // Pour "Avis" (index 5)
            { titleKey: "nav_settings", descKey: "nav_settings_desc" },
        ];
        return dashboardItems.map((item, index) => {
            const trans = translations[index];
            if (!trans) return item; // For items without translation keys (like Avis)
            
            const tTitle = t(trans.titleKey as any);
            const tDesc = t(trans.descKey as any);
            
            return {
                ...item,
                // If translation function returns the key itself, fallback to the hardcoded title
                title: tTitle === trans.titleKey ? item.title : tTitle,
                description: tDesc === trans.descKey ? item.description : tDesc,
            };
        });
    }, [t]);
        ...
        async function loadProfile() {
            try {
                ...
                if (result.success && 'user' in result && result.user) {
                    ...
                    setUserAvatar((result.user as any).avatar || null);
                    document.documentElement.dataset.userName = result.user.name || "Utilisateur";
                }
            } catch (err) {
                console.error("Impossible de charger le profil global", err);
            }
        }
        loadProfile();
    }, []);
        ...
        <FavoritesProvider>
            <div className="h-screen h-[100dvh] bg-background text-foreground flex flex-col md:flex-row overflow-hidden transition-colors duration-300">
                ...
                <aside
                    ...
                    } ${isCollapsed ? "w-24 md:w-28" : "w-80 md:w-80 lg:w-96"
                        }`}
                >
                    ...
                    <div className="flex flex-col h-full overflow-y-auto overscroll-contain no-scrollbar">
                        ...
                        {/* Persistent Branding Logo Area (Link to Dashboard) */}
                        <Link href="/role/user/dashboard" className={`mb-12 flex items-center transition-all duration-500 hover:opacity-80 active:scale-95 ${isCollapsed ? "justify-center" : "gap-1 pl-4"} ${
                            ...
                            <div className={`transition-all duration-500 flex flex-col ${isCollapsed ? "w-0 opacity-0 pointer-events-none" : "w-auto opacity-100 ml-2"}`}>
                                <h1 className={`${playfair.className} text-3xl italic tracking-tighter text-[#bca086] leading-none mb-1`}>aston</h1>
                        ...
                        <nav className="flex flex-col gap-3 mb-auto">
                            {translatedDashboardItems.map((item, index) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        href={item.href}
                                        onClick={() => setIsMobileMenuOpen(false)}
                                        className={`group relative flex items-center rounded-2xl transition-all duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] border ${
                                            isActive
                                            ...
                                            } ${isCollapsed ? "w-14 h-14 md:w-16 md:h-16 justify-center mx-auto" : "p-5 md:p-4 w-full"} ${
                    ...
                    <header className="h-20 border-b border-slate-100 dark:border-white/10 flex items-center justify-between md:justify-end px-4 md:px-10 bg-background/80 backdrop-blur-xl z-50 shrink-0">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="hidden lg:flex items-center gap-3 text-right">
                                <div>
                                    <h2 className="text-sm font-black uppercase tracking-wide leading-none">{userName}</h2>
                                    <p className="text-[10px] font-bold opacity-40 uppercase tracking-tighter mt-1">Client</p>
                                </div>
                                <div className="w-10 h-10 rounded-full bg-[#bca086] flex items-center justify-center text-white shadow-md border-2 border-white dark:border-black uppercase text-lg font-black italic overflow-hidden shrink-0">
                                    {userAvatar
