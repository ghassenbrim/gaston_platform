...
import { fetchUserProfileAction, updateUserProfileAction, updateUserPasswordAction } from "./actions";
...
function SettingsContent() {
    ...
    useEffect(() => {
        fetchUserProfileAction().then(result => {
            if (result.success && 'user' in result && result.user) {
                const u = result.user;
                setName(u.name || "");
                setEmail(u.email || "");
                setPhone((u as any).phone || "");
                setAge((u as any).age !== null && (u as any).age !== undefined ? String((u as any).age) : "");
                setGender((u as any).gender || "");
                setAvatar((u as any).avatar || null);
            }
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.success) showToast("Profil mis à jour avec succès !", "success");
    ...
    return (
        <div className="p-6 md:p-12 lg:p-16 space-y-10 relative">
            ...
            <div className="flex items-center gap-6 p-6 bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-white/5 shadow-sm max-w-sm">
                ...
                <div>
                    <p className="font-black text-slate-800 dark:text-white text-sm">{name || "Utilisateur"}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">Client Privé</p>
                    <p className="text-xs text-slate-400 mt-2">Photo de profil</p>
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-sm p-8 md:p-10 space-y-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-medium mt-0.5">Nom, téléphone, âge et genre</p>
                        </div>
                    </div>
                    ...
                    {loadingProfile ? (
                        ...
                        <form onSubmit={handleSaveProfile} className="space-y-5">
                            ...
                            <Field label="Numéro de téléphone" value={phone} onChange={setPhone} type="tel" placeholder="+33 6 00 00 00 00" />
                            <Field label="Âge" value={age} onChange={setAge} type="number" placeholder="Ex : 30" />
    ...
    return (
        <Suspense fallback={
            <div className="flex items-center justify-center min-h-screen">
                <div className="animate-spin h-12 w-12 border-4 border-[#bca086] border-t-transparent rounded-full shadow-lg"></div>
            </div>
        }>
            <SettingsContent />
        </Suspense>
