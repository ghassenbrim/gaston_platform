...
const CATEGORIES = ["Tous", "Mariage", "Anniversaire", "Immobilier", "Evenement", "Autre", "Général"];
                ...
                <div className="grid grid-cols-3 gap-4 md:gap-3">
                    {socials.map(s => (
                        <a
                            ...
                            onClick={onClose}
                            className={`${s.color} flex flex-col items-center justify-center gap-3 py-5 md:py-4 rounded-2xl text-white hover:opacity-90 active:scale-95 transition-all`}
                        >
                            <svg className="w-6 h-6 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 24 24">{s.icon}</svg>
                            <span className="text-[10px] md:text-[9px] font-black uppercase tracking-wide">{s.label}</span>
                        </a>
                    ))}
                </div>
...
export default function MesPhotosPage() {
    ...
    const handleDownload = useCallback(async (photo: FavoritePhoto, e: React.MouseEvent) => {
        ...
        await downloadPhoto(photo.url, `media-${photo.category}-${photo.id.slice(0, 6)}.${ext}`);
        setDownloading(null);
    }, []);

    const filtered = filterCat === "Tous" ? photos : photos.filter(p => p.category === filterCat);
    const usedCats = ["Tous", ...CATEGORIES.slice(1).filter(c => photos.some(p => p.category === c))];
        ...
        <div className="p-4 md:p-12 lg:p-20">
            ...
            {preview && (
                <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-md flex flex-col items-center justify-between p-4 md:p-8" onClick={() => setPreview(null)}>
                    ...
                    <div className="w-full flex items-center justify-between" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all hover:scale-110 active:scale-95 ${isFavorite(preview.id) ? "bg-rose-500 text-white" : "bg-white/10 text-white hover:bg-white/20"}`}
                            >
                                <svg className={`w-5 h-5 ${isFavorite(preview.id) ? "fill-current" : "fill-none"}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                            </button>
                            {/* Télécharger */}
                            <button
                    ...
                    <p className="text-white/30 text-xs">{preview.createdAt}</p>
                </div>
            )}

            {/* Header */}
            <header className="mb-12 space-y-4">
                <h1 className={`${playfair.className} text-4xl sm:text-5xl md:text-7xl italic text-slate-900 dark:text-white`}>Ma Galerie</h1>
                <p className="text-xl text-slate-500 font-light max-w-2xl leading-relaxed">Consultez, téléchargez et partagez vos photos et vidéos.</p>
            </header>

            {loading ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {Array.from({ length: 8 }).map((_, i) => <div key={i} className="aspect-square rounded-[2rem] bg-slate-100 dark:bg-zinc-800 animate-pulse" />)}
                </div>
            ) : photos.length === 0 ? (
                <div className="border-2 border-dashed border-slate-100 dark:border-white/5 rounded-[2.5rem] flex flex-col items-center justify-center py-32 text-slate-400 bg-slate-50/30 dark:bg-zinc-900/10">
                    ...
                    <p className="font-bold uppercase text-[10px] tracking-[0.2em]">Aucun média pour l&apos;instant</p>
                    <p className="text-sm mt-2 opacity-60">Vos photos et vidéos partagées par l&apos;équipe apparaîtront ici.</p>
                </div>
            ) : (
                <>
                    {/* Filtres */}
                        ...
                        <div className="flex flex-wrap gap-2 mb-8">
                            {usedCats.map(cat => (
                                <button key={cat} onClick={() => setFilterCat(cat)}
                                    ...
                                    {cat}{cat !== "Tous" && <span className="ml-1.5 opacity-60">({photos.filter(p => p.category === cat).length})</span>}
                                </button>
                            ))}
                        </div>
                    )}

                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6">{filtered.length} média{filtered.length !== 1 ? "s" : ""}</p>

                    {/* Grille */}
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-4">
                        {filtered.map(photo => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300">
                                        ...
                                        <div className="absolute top-3 right-3 flex flex-col gap-2">
                                            ...
                                            <button
                                                ...
                                                className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 shadow-lg ${fav ? "bg-rose-500 text-white" : "bg-white/90 text-slate-600 hover:bg-rose-500 hover:text-white"}`}
                                                title="Ajouter aux favoris"
                                            >
                                                <svg className={`w-4 h-4 ${fav ? "fill-current" : "fill-none"}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                                            </button>
                                            {/* Télécharger */}
                                            <button
                                                onClick={e => handleDownload(photo, e)}
                                                disabled={dlding}
                                                className="w-11 h-11 rounded-xl bg-white/90 text-slate-600 flex items-center justify-center hover:bg-[#bca086] hover:text-white transition-all hover:scale-110 active:scale-95 shadow-lg disabled:opacity-60"
                                                title="Télécharger"
                                            >
                                                {dlding
                                            ...
                                            <button
                                                ...
                                                className="w-11 h-11 rounded-xl bg-white/90 text-slate-600 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all hover:scale-110 active:scale-95 shadow-lg"
                                                title="Partager"
                                            >
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
                                            </button>
                                    ...
                                    {/* Badge favori permanent si actif */}
                                    {fav && (
                                        <div className="absolute top-3 left-3 w-7 h-7 rounded-xl bg-rose-500 flex items-center justify-center shadow-lg">
